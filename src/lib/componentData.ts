...
<style>
.pill-switch { width: 140px; height: 60px; background: var(--color-border-light); border-radius: 9999px; position: relative; display: flex; justify-content: space-between; align-items: center; padding: 0 14px; cursor: pointer; font-family: sans-serif; }
...
<div style="display:flex;flex-direction:column;font-family:sans-serif;padding:24px;background:var(--color-bg-island);border:1px solid var(--color-border-light);border-radius:24px;max-width:250px">
  <span style="color:#52525b;font-weight:600;font-size:0.9rem">Total Volume</span>
  <div style="font-size:2.2rem;font-weight:800;color:var(--color-text-main);display:flex;align-items:center;gap:8px;margin-top:8px">24.8k
...
.tooltip-box { position: absolute; bottom: calc(100% + 12px); left: 50%; transform: translateX(-50%) translateY(10px); background: var(--color-bg-island); opacity: 0.95; backdrop-filter: blur(12px); border: 1px solid var(--color-border-light); color: var(--color-text-main); padding: 10px 16px; border-radius: 8px; font-size: 0.85rem; font-weight: 600; white-space: nowrap; pointer-events: none; transition: all 0.2s cubic-bezier(0.34, 1.56, 0.64, 1); opacity: 0; }
.tooltip-container:hover .tooltip-box { opacity: 1; transform: translateX(-50%) translateY(0); }
</style>
<div class="tooltip-container">
  <button style="background:var(--color-bg-island);border:1px solid var(--color-border-light);padding:12px 24px;border-radius:99px;font-weight:600;cursor:pointer;color:var(--color-text-main)">Hover Me</button>
...
<div style="width:100%;max-width:300px;height:8px;background:var(--color-border-light);border-radius:99px;overflow:hidden;position:relative">
...
<style>
.mini-accordion { width: 100%; max-width: 300px; background: var(--color-bg-island); border: 1px solid var(--color-border-light); border-radius: 16px; padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; font-weight: 600; font-family: sans-serif; cursor: pointer; color: var(--color-text-main); }
.mini-accordion:hover { background: var(--color-surface-light); }
...
<style>
.gradient-hover-text { font-size: 3rem; font-weight: 900; letter-spacing: -2px; color: var(--color-text-main); cursor: pointer; font-family: sans-serif; display: inline-block; }
...
<style>
.spotlight-card { position: relative; overflow: hidden; background: var(--color-bg-island); border: 1px solid var(--color-border-light); border-radius: 24px; padding: 40px; text-align: center; cursor: crosshair; }
...
<script>
const phrases=['npm install runflow-ui','npx create-app@latest','git push origin main'];
